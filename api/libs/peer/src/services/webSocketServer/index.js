...
class WebSocketServer extends events_1.default {
    ...
    _onSocketConnection(socket, req) {
        ...
        var clientToken = new URLSearchParams(req.url).get('clientToken');
        this._registerClient({ socket, id, token, clientToken });
    }
    _onSocketError(error) {
        // handle error
        this.emit("error", error);
    }
    _registerClient({ socket, id, token, clientToken }) {
        // Check concurrent limit
        const clientsCount = this.realm.getClientsIds().length;
        if (clientsCount >= this.config.concurrent_limit) {
            return this._sendErrorAndClose(socket, enums_1.Errors.CONNECTION_LIMIT_EXCEED);
        }
        const newClient = new client_1.Client({ id, token, clientToken });
